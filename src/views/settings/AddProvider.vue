<template>
  <section>
    <v-card-text>
      <!-- header -->
      <div v-if="config && availableProviders[config.domain]" style="margin-left: -5px; margin-right: -5px">
        <v-card-title>
          {{ $t('settings.setup_provider', [availableProviders[config.domain].name]) }}
        </v-card-title>
        <v-card-subtitle> {{ availableProviders[config.domain].description }} </v-card-subtitle><br />
        ...
        <v-card-subtitle v-if="availableProviders[config.domain].documentation">
          ...
          <a :href="availableProviders[config.domain].documentation" target="_blank">{{ $t('settings.check_docs') }}</a>
        </v-card-subtitle>
      </div>
      <br />
      <v-divider />
      <br />
      <br />
      <edit-config v-if="config" :model-value="config" @update:model-value="onSubmit($event as ConfigUpdate)" />
    </v-card-text>
  </section>
</template>
