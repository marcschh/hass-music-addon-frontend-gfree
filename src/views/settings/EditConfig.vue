  ...
  <section>
    <v-form v-if="conf" ref="form" v-model="valid" style="margin-right: 10px">
      ...
      <v-text-field
        ...
        :label="'player_id' in conf ? $t('settings.player_name') : $t('settings.provider_name')"
        variant="outlined"
        clearable
      />
      ...
      <v-switch
        ...
        :label="'player_id' in conf ? $t('settings.enable_player') : $t('settings.enable_provider')"
        color="primary"
        :disabled="isBuiltin"
      />
      <v-divider />
      <!-- config rows for all config entries -->
      <v-expansion-panels v-if="conf.enabled" v-model="activePanel" variant="accordion" multiple>
        <!-- 
            ...
            >
              <div class="configcolumnleft">
                ...
                <div v-if="conf_item_value.type == ConfigEntryType.DIVIDER">
                  <br />
                  <v-divider />
                  <v-label v-if="conf_item_value.value" style="margin-left: 8px; margin-top: 10px; margin-bottom: 10px">
                ...
                <v-switch
                  ...
                  color="primary"
                  :disabled="conf_item_value.depends_on != undefined && !conf.values[conf_item_value.depends_on].value"
                />
                ...
                <v-text-field
                  ...
                  :required="conf_item_value.required"
                  :disabled="conf_item_value.depends_on != undefined && !conf.values[conf_item_value.depends_on].value"
                  :rules="[(v) => !(!v && conf_item_value.required) || $t('settings.invalid_input')]"
                ...
                <!-- value with dropdown -->
                <v-select
                  v-else-if="conf_item_value.options && conf_item_value.options.length > 0"
                  v-model="conf_item_value.value"
                ...
                <v-text-field
                  v-else-if="
                    conf_item_value.type == ConfigEntryType.INTEGER || conf_item_value.type == ConfigEntryType.FLOAT
                  "
                  v-model="conf_item_value.value"
                  :placeholder="conf_item_value.default_value?.toString()"
                  :disabled="conf_item_value.depends_on != undefined && !conf.values[conf_item_value.depends_on].value"
...
const hasAdvanced = computed(() => {
  ...
  return Object.values(conf.value.values).filter((x) => x.advanced && !x.hidden).length > 0;
});
